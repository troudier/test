...
class RequetteurService
{
    ...
    public function buildSegmentRequete($filtres, $index = null, $count = false, $prive = false)
    {
        $sql =
            $count
                ...
                :
                'SELECT distinct(pl_uuid) as uuid, pl.pl_libelle, pl.pl_visibilite as visibilite
        ...
        foreach ($filtresOrdonnes as $i => $filtre) {
            if (!$index || $i <= (int) $index) {
                if (json_decode($filtre->getChamp()->getJoin(), true)) {
    ...
    public function countContacts($segment, $index, $prive = false)
    {
